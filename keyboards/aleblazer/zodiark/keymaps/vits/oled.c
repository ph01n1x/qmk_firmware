...
{    
    ...
    static const char PROGMEM awake[FRAMES][FRAME_SIZE] = 
    {
        {
        // 'awake', 32x32px
    ...
    static const char PROGMEM yawn[FRAMES][FRAME_SIZE] =
    {
        {        
        // 'mati2', 32x32px
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x30, 0x08, 0x10, 0x60, 0x80, 
    ...
    {
        // First animation after sleep will always be awakt
        ...
        switch (current_animation)
        {
            case ANIMATION_AWAKE:               
                ...
                image = awake[frameIndex];
                
                // Awake goes back to run
            ...
            case ANIMATION_NORMAL:               
                current_frame = (current_frame + 1) % FRAMES;
                frameIndex = abs((FRAMES - 1) - current_frame);
                image = mati[frameIndex];
                break;
    
            case ANIMATION_KAKI:
                current_frame = (current_frame + 1) % FRAMES;
                frameIndex = abs((FRAMES - 1) - current_frame);
                image = kaki[frameIndex];
                break;
                
            case ANIMATION_YAWN:
                current_frame = (current_frame + 1) % FRAMES;
                frameIndex = abs((FRAMES - 1) - current_frame);
                image = yawn[frameIndex];
                break;                
